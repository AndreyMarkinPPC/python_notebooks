...
def get_jetson_gstreamer_source(capture_width=800, capture_height=600, display_width=800, display_height=600, framerate=30, flip_method=0):
    """
...
def main_loop():
    ...
    while True:
        ...
        face_labels = []
        for face_location, face_encoding in zip(face_locations_, face_encodings_):
        ...
        for (top, right, bottom, left), face_label in zip(face_locations_, face_labels):
        ...
        for (top, right, bottom, left) in face_locations_:
            ...
            # Draw a label with a name below the face
            cv2.rectangle(frame, (left, bottom - 35), (right, bottom), (0, 0, 255), cv2.FILLED)
            cv2.putText(frame, "hello", (left + 6, bottom - 6), cv2.FONT_HERSHEY_DUPLEX, 0.8, (255, 255, 255), 1)
        if len(face_locations_) > 0:
            cv2.putText(frame, "Visitors at Door", (5, 18), cv2.FONT_HERSHEY_DUPLEX, 0.8, (255, 255, 255), 1)
        ...
        if len(face_locations_) > 0 and number_of_faces_since_save > 10:
            save_known_faces()
            number_of_faces_since_save = 0
        else:
